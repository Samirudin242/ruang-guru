...
import prizeImg from "../img/englishPrize.png";
  ...
  useEffect(() => {
    async function fetchData() {
      const result = await Axios({
        method: "GET",
        url: "http://localhost:3000/package/englishacademy",
      }).then((data) => {
        return data;
      });
  ...
  if (!datas) {
    ...
    return (
      <div className="mt-5">
        ...
        <h2 className="mb-4 d-flex justify-content-center">English Academy</h2>
        <Row>
          {datas.map((data) => {
            return (
              <Col key={data.id}>
                <Card style={{ width: "18rem" }}>
                  <Card.Img
                    style={{ height: "300px" }}
                    variant="top"
                    src="https://www.ruangguru.com/hubfs/1%20Homepage%20RG/EA_newwww.png"
                  />
                  <Card.Body>
